...
, IMM_DAT AS (   
  SELECT    distinct 
            ...
            , ''                           org_id    ---- this will be  vtrcks_prov_pin -- parent vfc  
            , CASE                           
                  WHEN  ENC_DEP.specialty_dep_c IN ( 9 )   THEN  'Medical practice : family medicine' 
                  WHEN  ENC_DEP.specialty_dep_c IN ( 17 )  THEN  'Medical practice : internal medicine'                                     
                  ELSE                                           'Medical practice : other specialty' 
              END                           admin_type    ---- this will be vfc for admin  location 
                           
            , ENC_DEP.DEPARTMENT_NAME	      admin_name
            
           ------------------------------------------------------------------------------------ 
           -- BELOW LOGIC IS TEMPORARY -- WE HAVE NOT RECEIVED THE ACTUAL VFC for admin  location 
           ------------------------------------------------------------------------------------                            
           , enc_dep.department_id ||' ' ||  enc_dep.department_NAME   AS    xxxxxEPIC_DEPARTMENTxxxxx

           , ''                           admin_id   --- vfc for admin  location  '40166742'
           , ''                           vtrcks_prov_pin   --- vfc for admin  location  '40166742' 
           , NDC_CODE                      ndc 
           ,CAST(IMMUNE.IMMUNE_DATE AS DATE)  IMMUNE_DATE
 
             
------------------------------------------------------------------------------------
--  PER MOSES - BE PREPARED TO USE   immunization_time 
--
-- immune.immunization_time is not populated on each admin record - use entry_time which is populated and is close to the immunization time.                                
--- , TO_CHAR( COALESCE (  immune.immunization_time , immune.entry_dttm  )    , 'DD-MON-YYYY HH24:MI') admin_date
 ------------------------------------------------------------------------------------ 
            ...
            , ZC_SITE.NAME                    vax_admin_site
            , ZC_ROUTE.NAME                   vax_route 
...
---- USE COALESCE STATEMENT FOR IMM AND ORD TO HELP IDENTIFY LOCATION TO USE FOR VAX 
          ...
          LEFT JOIN CLARITY_SER               ON CLARITY_SER.USER_ID             = EMP.USER_ID                ---GIVEN_BY_USER
  ...
  WHERE  1=1    
   -- AND  IMMUNE.IMMUNE_ID   ='23377136'
    AND  IMMUNE.IMMNZTN_STATUS_C = 1  ---  GIVEN 
--    AND  (  IMMUNE.IMM_HISTORIC_ADM_YN IS NULL OR IMMUNE.IMM_HISTORIC_ADM_YN ='N'  )
                          ...
                          SELECT  DISTINCT
                            ...
                            FROM ( 
                                   SELECT  DISTINCT 
                                           BASE_POP.PAT_ID
                                     FROM  BASE_POP 
                                    WHERE (   BASE_POP.IMM_HISTORIC_ADM_YN IS NULL 
                                           OR BASE_POP.IMM_HISTORIC_ADM_YN ='N'
                                            )                            
                                     ) BASE_POP
...
--------------------------------------------------------------------------------------------------------------------------------------------------------------                  
 ,COMORBIDITY AS ( 
                  SELECT  X.PAT_ID 
                        , COUNT ( X.REGISTRY_ID) COMORBIDITIES
                    FROM BASE_POP
                   LEFT JOIN (
                                SELECT DISTINCT   
                                       PAT_ACTIVE_REG.PAT_ID 
                                  ...
                                  FROM ( 
                                        ...
                                        JOIN  PAT_ACTIVE_REG   ON    BASE_POP.PAT_ID    =     PAT_ACTIVE_REG.PAT_ID    AND PAT_ACTIVE_REG.REGISTRY_ID  = 	82030  -- CANCER POPULATION REGISTRY
                                UNION
                                SELECT DISTINCT   
                                       PAT_ACTIVE_REG.PAT_ID 
                                     , PAT_ACTIVE_REG.REGISTRY_ID  
                                  ...
                                  FROM ( 
                                        ...
                                        JOIN  PAT_ACTIVE_REG   ON    BASE_POP.PAT_ID    =     PAT_ACTIVE_REG.PAT_ID    AND PAT_ACTIVE_REG.REGISTRY_ID  = 	82005 ----Chronic kidney disease
                                UNION                                
                                SELECT DISTINCT   
                                       PAT_ACTIVE_REG.PAT_ID 
                                     , PAT_ACTIVE_REG.REGISTRY_ID  
                                  ...
                                  FROM ( 
                                        ...
                                        JOIN  PAT_ACTIVE_REG   ON    BASE_POP.PAT_ID    =     PAT_ACTIVE_REG.PAT_ID    AND PAT_ACTIVE_REG.REGISTRY_ID  = 	82009 --COPD (chronic obstructive pulmonary disease)
                                UNION                                
                                SELECT DISTINCT   
                                       PAT_ACTIVE_REG.PAT_ID 
                                     , 82004    
                                  FROM ( 
                                        ...
                                        JOIN  PAT_ACTIVE_REG   ON    BASE_POP.PAT_ID    =     PAT_ACTIVE_REG.PAT_ID    AND PAT_ACTIVE_REG.REGISTRY_ID  = 	82009 --COPD (chronic obstructive pulmonary disease)
                                 WHERE  REGISTRY_ID IN (---Heart conditions, such as heart failure, coronary artery disease, or cardiomyopathies
...
----Immunocompromised state (weakened immune system) from solid organ transplant            
                                ...
                                SELECT DISTINCT   
                                  ...
                                  FROM ( 
                                        ...
                                        JOIN  PAT_ACTIVE_REG   ON    BASE_POP.PAT_ID       = PAT_ACTIVE_REG.PAT_ID  AND PAT_ACTIVE_REG.REGISTRY_ID    = 	82007	                    	----OBESITY REGISTRY  
                                        JOIN  DM_OBESITY       on  PAT_ACTIVE_REG.PAT_ID   = DM_OBESITY.pat_id  
                                       WHERE  DM_OBESITY.BMI_LAST >= 34  
                                            
                                UNION
                                      SELECT DISTINCT   BASE_POP.pat_id , 4  ---PREG  
                                        FROM BASE_POP
                                             JOIN  DM_WLL_ALL     on   DM_WLL_ALL.pat_id             =  BASE_POP.pat_id
                                ...
                                SELECT DISTINCT   
                                  ...
                                  FROM ( 
                                         ...
                                         JOIN  PAT_ACTIVE_REG   ON BASE_POP.PAT_ID        = PAT_ACTIVE_REG.PAT_ID  AND  PAT_ACTIVE_REG.REGISTRY_ID = 	82014	-- TOBACCO REGISTRY  --smokers 
                                         JOIN  DM_WLL_ALL       on PAT_ACTIVE_REG.PAT_ID  = DM_WLL_ALL.pat_id  
                                       WHERE (  DM_WLL_ALL.SMOKING_STATUS_C IN  ( --ZC_SMOKING_TOB_USE 
...
---------method 1) use SINGLE TABLE DM_COVID_CONFIRMED 
              select 
                       DM_COVID_CONFIRMED.PAT_ID
                      ,pos_sars_cov2_last  RESULT
                      ,pos_sars_cov2_last_dt  RESULT_DATE
                from ( SELECT DISTINCT  PAT_ID FROM BASE_POP ) BASE_POP
                JOIN DM_COVID_CONFIRMED   ON BASE_POP.PAT_ID  =    DM_COVID_CONFIRMED.PAT_ID
               where pos_sars_cov2_last  ='Positive' 
                 and pos_sars_cov2_last_dt  BETWEEN   ( SELECT START_DATE FROM MYPARAMS )    and    ( SELECT END_DATE FROM MYPARAMS )
               --and pat_id in (  'Z969207' )  
--              --   
...
--               
--              from    ( SELECT DISTINCT BASE_POP.PAT_ID FROM BASE_POP ) pat_ids
--                     join  RES_DB_MAIN RES_DB               on pat_ids.pat_id   =   res_db.RES_EPT_PAT_ID  
...
--                 
--              from   ( SELECT DISTINCT BASE_POP.PAT_ID FROM BASE_POP ) pat_ids 
--                    join order_proc ord                 on pat_ids.pat_id    =  ord.PAT_ID   
...
, ADMIN_IMMS AS  ( 
                   ...
                   pat_demo.pat_id,
                            ...
                            , BASE_POP.org_name     
                            , BASE_POP.org_id    ---- this will be  vtrcks_prov_pin -- parent vfc  
                            , BASE_POP.admin_type                                                                
                            , BASE_POP.admin_name
--          , BASE_POP.xxxxxEPIC_DEPARTMENTxxxxx   -----TESTING ONLY
                            , BASE_POP.admin_id   --- vfc for admin  location  '40166742'
                            ...
                            , BASE_POP.dose_num                            
                            , BASE_POP.lot_number   
                            , BASE_POP.vax_expiration
                            , BASE_POP.vax_admin_site
                            , BASE_POP.vax_route 
                            , BASE_POP.vax_admin_provider_name
                            , BASE_POP.Provider_License_Number 
                            , BASE_POP.vis_publication_date       -- SHEET GIVEN TO recipient DATE
                            , BASE_POP.vis_date_given_to_recipient   -- SHEET GIVEN TO recipient
                            , BASE_POP.vax_reaction       ----Reaction after Vaccination          --- dropped per updated spec 1/25/2021
                            , BASE_POP.vax_reaction_desc -------Adverse Reactions (Description)   --- dropped per updated spec 1/25/2021
...
--COVID-19 Vaccination Program in Five Phases
                            ...
                            ,CASE
                                  WHEN   trunc( months_between( BASE_POP.IMMUNE_DATE,  PAT_DEMO.birth_date )/ 12 ) >=65      THEN 'Phase 1b' 
                                  WHEN  (trunc( months_between( BASE_POP.IMMUNE_DATE,  PAT_DEMO.birth_date )/ 12 ) between 16 and 64 )
                                         and COMORBIDITY.COMORBIDITIES >0                                                    THEN 'Phase 3'                                   
                                  ELSE                                                                                            'Phase 4' 
                             END                                    recip_priority_group      --- ADD LOGIC  FOR TIER IE priority 
-----------------------------------------------------------   
  
                            , CASE 
--                                   WHEN serology.RESULT     IS NULL THEN 'UNK'  --- CHECK MEDICAL HX FOR COVID
