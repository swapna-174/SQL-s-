...
, IMM_DAT AS (    
  SELECT    distinct 
            ...
            , COALESCE  ( IMMUNE.IMM_CSN , ORDER_PROC.PAT_ENC_CSN_ID )            PAT_ENC_CSN_ID   -- ENSURE LINK BETWEEN IMMUNIZATION AND VISIT ENCOUNTERS
...
-- 
            ...
            , initcap( CASE WHEN ( CLARITY_POS.CITY ='WINSTON SALEM'  AND zc_cty.NAME IS NULL ) THEN 'Forsyth'
                            else  zc_cty.NAME  
                       end
                        )    DEP_COUNTY  
--            , CASE WHEN IMMUNE.IMM_CSN IS NULL THEN 'X'  END                      FAILED_iMM_CSN_CHECK
...
--          LEFT JOIN PAT_ENC  	  ORD_ENC       ON ORD_ENC.PAT_ENC_CSN_ID          = ORDER_PROC.PAT_ENC_CSN_ID              
          LEFT JOIN CLARITY_DEP ENC_DEP       ON  IMM_ENC.DEPARTMENT_ID          =  ENC_DEP.DEPARTMENT_ID   --- COALESCE ( IMM_ENC.DEPARTMENT_ID , ORD_ENC.DEPARTMENT_ID )   =  ENC_DEP.DEPARTMENT_ID 
...
, BASE_POP AS (
                SELECT * 
                 ...
                 WHERE DOSE_RANK = 1                        ---- LOOKUP THE MOST RECENT VAX TO FIND DOSE NUMBER
                   ...
                   AND  TRUNC (IMM_DAT.IMMUNE_DATE) BETWEEN   ( SELECT REPORT_BEGIN_DATE FROM MYPARAMS )    and    ( SELECT END_DATE FROM MYPARAMS )   ---COLLECT AND RANK VACCINATIONS
                 )
