...
--                , vlb.CASE_ID
                ...
                , pat.PAT_NAME
                , pat.BIRTH_DATE
                , ROUND(enc.WEIGHT * .0283495) "Weight in Kg"
...
--                        AND ordrout.SEQ_NUM_out = 1
                    ...
                    LEFT OUTER JOIN PAT_OR_ADM_LINK lnk ON vlb.LOG_ID =lnk.LOG_ID
                ...
                ORDER BY 
                    vlb.PRIMARY_PHYSICIAN_ID
                    , CASE WHEN olc.TRACKING_EVENT_C = '80' THEN olc.TRACKING_TIME_IN END
    )
)     
...
FROM
    (SELECT
        orl1.LOG_ID
        , CASE
            WHEN (orl1.PROCSTART1 >= orl1.START_TIME AND orl1.END_TIME IS NULL)
                OR (orl1.PROCSTART1 >= orl1.START_TIME AND orl1.PROCSTART1 <= orl1.END_TIME)
            THEN 'YES'
            ELSE 'NO'
        END AS Present
    FROM ORLOG1 orl1
...
SELECT 
    ...
    , RANK () OVER (PARTITION BY orl1.LOG_ID  ORDER BY orl1.START_TIME) AS SEQ_NUM
    --, orl1.SEQ_NUM_in
    --, orl1.SEQ_NUM_out
    --, orl1.PAT_ID "Pat ID"
    , orl1.PAT_MRN_ID "MRN"
    ...
    , CASE
        WHEN srg.LOG_ID IS NOT NULL THEN  'Yes'
        ELSE 'No'
    END AS "Surgeon Present at Incision?"
...
FROM ORLOG1 orl1
    ...
    LEFT OUTER JOIN SURGEON_PRESENT srg ON orl1.LOG_ID = srg.LOG_ID
