...
--    , peh.hosp_disch_time
--    , ord.ORDER_PROC_ID
--    , ser_atnd.PROV_NAME as current_attending
...
--    , round(  (cast(ord.PROC_ENDING_TIME as date) - cast(ord.PROC_START_TIME as date)) * 24  ) as diff_hours
    , round(round(((ord.PROC_ENDING_TIME - ord.PROC_START_TIME)*1440),0)/60,2) AS elapsed_time_hours
--    , ord.ORDERING_DATE
--    , ord.ORDER_INST
--    , zc_ord.name
--    , ord_summ.LINE
...
--    , ord_audit.audit_trl_user_id
    , ser_canc.PROV_NAME as order_discontinued_by
...
--    and ord.order_proc_id = '443123895'
    ...
    and trunc(ord.PROC_START_TIME) >= '15-AUG-2017' and trunc(ord.PROC_START_TIME) <= '20-AUG-2017'
...
--    and cl_q.quest_id IN ('100122') -- , '100123')
    and cl_q.quest_id IN ('100123', '101572', '152600', '152564',
                                            '101579', '152565', '152587','152588', '152589', '152590', '152591', '152565') -- , '100123')
