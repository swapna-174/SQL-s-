...
from bal_ndc_balance bal_ndc 
    ...
    left outer join rx_med_two rx2 on RXS.MEDICATION_ID = rx2.MEDICATION_ID 
    left outer join rx_med_one rx1 on RXS.MEDICATION_ID = rx1.MEDICATION_ID 
    left outer join clarity_medication cm on RXS.MEDICATION_ID = cm.MEDICATION_ID 
...
select distinct 
 ndc.RAW_11_DIGIT_NDC "DrugIdentifier",
    rx2.MEDICATION_NAME "DrugName",
    To_Char(cm.GPI) "GenericEquivalencyCode",
...
from bal_trail bal_trl 
    ...
    inner join     ----- Added 10/7/21 to capture NDC when NDC Default is 'N' or Null
    (
      ...
      select *
        ...
        ,ROW_NUMBER() OVER (PARTITION BY bal_ndc.BAL_ID ORDER BY bal_ndc.BAL_ID, bal_ndc.NDC_DEFAULT_DISP_YN, bal_ndc.NDC_BALANCE,bal_ndc.NDC_ID) AS DocsRow
        from bal_ndc_balance bal_ndc
        Inner join BAL_PRD_BALANCE bal_prd on bal_ndc.BAL_ID = bal_prd.BAL_ID 
        inner join RX_INVENTORY_LOC loc on bal_prd.INV_MASTER_LOC_ID = loc.INVENTORY_LOC_ID 
        inner join RX_PHR phr on loc.PHR_ID = phr.PHARMACY_ID and phr.npi= 1073732376
       ) 
      where   DocsRow = 1
    ...
    inner join PRD_MEDICATIONS prd_med on prd_info.RECORD_ID = prd_med.RECORD_ID 
    left outer join prd_package_sizes pac_sz on prd_info.RECORD_ID = pac_sz.RECORD_ID 
    left outer join rx_med_two rx2 on prd_med.MEDICATION_ID = rx2.MEDICATION_ID 
    ...
    left outer join zc_med_unit zcmu on ndc.MED_UNIT_C = zcmu.DISP_QTYUNIT_C 
    where 
    bal_trl.UPDATE_DATETIME >= Trunc(Add_Months(epic_util.efn_din(SysDate), -24), 'MM') 
    and bal_trl.UPDATE_REASON_C = 1 ---- Dispense
  and bal_trl.BAL_ID = 1072426
