...
-- EXCLUDE WHERE OLD_PROV_ID =  '8274'	--UNITED HEALTH CENTERS @ PETERS CREEK PARKWAY
         INNER JOIN (  -- NOT LEFT OUTER
                       SELECT 
                         ...
                         FROM (
                                 SELECT DISTINCT  
                                         pat_enc_chng_appt.pat_enc_csn_id
                                        ,tmpser.prov_id    
                                        ,tmpser.prov_name  
                                        ,tmpser.prov_type 
                                      , ROW_NUMBER() 
                                             OVER ( PARTITION BY  pat_enc_chng_appt.pat_enc_csn_id,tmpser.prov_id   
                                                        ORDER BY  pat_enc_chng_appt.pat_enc_csn_id,tmpser.prov_id    , contact_date  DESC  )        SER_rank  
                                   FROM   pat_enc_chng_appt           
                                          LEFT JOIN clarity_ser tmpser  on  pat_enc_chng_appt.old_prov_id = tmpser.prov_id  
                                  WHERE  1=1  
                                  AND  ( tmpser.prov_id IS NULL OR (    pat_enc_chng_appt.NEW_PROV_ID  <> '8274' 
                                                                     AND pat_enc_chng_appt.old_prov_id <> '8274' )
                                                                     )                                    
                                )  
                         WHERE SER_RANK =1
...
--                            
--,      COALESCE (   GRP.employment_STATUS ,PAT_DEMO.EMPLOYMENT )     EMPLOYMENT
-- 
--,     CASE 
...
,     CASE 
            ...
            WHEN  UPPER ( COALESCE (  GRP.INDUSTRY  ,PAT_DEMO.INDUSTRY  ) ) IS NULL THEN    'None of the Above'  ---- 'Other / Not Applicable'  
            ELSE    'None of the Above'
       END       INDUSTRY
