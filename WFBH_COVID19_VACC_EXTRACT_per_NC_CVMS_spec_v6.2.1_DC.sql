...
WITH MYPARAMS AS (  
                    SELECT  
                               TRUNC( EPIC_UTIL.EFN_DIN( '12-MAR-2021' ))  AS START_DATE     
                             , TRUNC( EPIC_UTIL.EFN_DIN( '14-MAR-2021' ))  AS END_DATE  
                     FROM   DUAL   
                     ) 
...
--            ,IMMUNE.IMM_PRODUCT 
             ,clarity_lot.product product_name
            ...
            , CLARITY_POS.CITY  DEP_CITY
            , CLARITY_POS.ZIP   DEP_ZIP
            , initcap( zc_cty.NAME   )    DEP_COUNTY  
            , cp.PRC_ID
...
---- USE COALESCE STATEMENT FOR IMM AND ORD TO HELP IDENTIFY LOCATION TO USE FOR VAX 
          LEFT JOIN ORDER_PROC                ON IMMUNE.ORDER_ID                 = ORDER_PROC.ORDER_PROC_ID
                                              ...
                                              AND  ORDER_PROC.PROC_ID in (  --- ORDERED VACC
                                                                          ...
                                                                          ,'149106' -----  JOHNSON and JOHNSON - JANSSEN ----  IMM206
...
--		This code examines table APPT_UTIL_SNAPSHOT for the existence of generic provider 8274 (UNITED HEALTH CENTER) and UTIL_SNAP_CHANGE has a value of 0
  ...
  WHERE  1=1 
    ...
    and imm_enc.department_id = '1000100025'
              )
...
--                           ,BASE_POP.last_update                                  ---2021_02_09 new field for adding update
                        ...
                        ,BASE_POP.PAT_ID  
                        ,case when BASE_POP.PRODUCT_NAME   like 'Pfizer%'  then BASE_POP.PRODUCT_NAME 
                              when BASE_POP.PRODUCT_NAME   like 'Moderna%' then BASE_POP.PRODUCT_NAME
                              when BASE_POP.PRODUCT_NAME   like 'Jen%' then BASE_POP.PRODUCT_NAME
                              when BASE_POP.PRODUCT_NAME   like 'John%' then BASE_POP.PRODUCT_NAME  
                              else  
                                   case
                                        when BASE_POP.ndc like '59267%' then 'Pfizer COVID-19 Vaccine (EUA)'
                                        when BASE_POP.ndc like '80777%' then 'Moderna COVID-19 Vaccine (EUA)'
                                        when BASE_POP.ndc like '59676%' then 'Johnson Johnson COVID-19 Vaccine (EUA)'
