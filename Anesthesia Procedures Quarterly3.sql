...
--    LEFT OUTER JOIN CLARITY_CONCEPT cc ON sed.ELEMENT_ID = cc.CONCEPT_ID
...
WHERE fans.AN_DATE > '18-Sep-2019'
    AND fans.AN_DATE < '21-Sep-2019'
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Provider 1 ID"
    , sed.ELEMENT_ID
    , ser.PROV_NAME "Provider 1 Name"
    , ser.PROV_TYPE "Provider 1 Type"
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    INNER JOIN CLARITY_SER ser ON sev.SMRTDTA_ELEM_VALUE = ser.PROV_ID
WHERE sed.ELEMENT_ID = 'EPIC#13218'
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Provider 2 ID"
    , sed.ELEMENT_ID
...
FROM PAT_POP pp
    ...
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    INNER JOIN CLARITY_SER ser ON sev.SMRTDTA_ELEM_VALUE = ser.PROV_ID
WHERE sed.ELEMENT_ID = 'EPIC#13219'
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Provider 3 ID"
    , sed.ELEMENT_ID
...
FROM PAT_POP pp
    ...
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    INNER JOIN CLARITY_SER ser ON sev.SMRTDTA_ELEM_VALUE = ser.PROV_ID
WHERE sed.ELEMENT_ID = 'EPIC#13190'
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Patient Position"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#PROC0012'
),
...
(SELECT
    ...
    , sed.ELEMENT_ID
    , CASE
        WHEN sed.ELEMENT_ID = 'EPIC#18418' THEN 'Chloraprep'
        WHEN sed.ELEMENT_ID = 'EPIC#12843' THEN 'DuraPrep'
        WHEN sed.ELEMENT_ID = 'EPIC#22635' THEN 'Betadine'
    END AS "Patient Prep"
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID IN ('EPIC#18418', 'EPIC#12843', 'EPIC#22635')
),
...
FROM
    (SELECT
        pp."Log ID"
        , pp."Order ID"
        , sev.LINE "Sterile Prep"
        , sev.SMRTDTA_ELEM_VALUE "Asepsis Value"
        , sed.ELEMENT_ID
    FROM PAT_POP pp
        INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
        INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    WHERE sed.ELEMENT_ID = 'EPIC#50553'
    )
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Laterality"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#72501'
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Block Type"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#12678'
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Technique"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#72549'
),
...
FROM
    (SELECT
        pp."Log ID"
        , pp."Order ID"
        , sev.SMRTDTA_ELEM_VALUE "Location Value"
        , sed.ELEMENT_ID "Concept ID"
        , CASE
            WHEN sed.ELEMENT_ID = 'EPIC#72506' THEN 'Nerve Stimulation'
            WHEN sed.ELEMENT_ID = 'EPIC#72531' THEN 'Ultrasound'
            WHEN sed.ELEMENT_ID = 'EPIC#14412' THEN 'Landmark'
        END AS  "Needle Localization"
    FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    WHERE sed.ELEMENT_ID IN ('EPIC#72506', 'EPIC#72531', 'EPIC#14412')
    )
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE "Ultrasound Orientation"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID IN ('EPIC#15426', 'WHANE#014')
),
...
(SELECT
    pp."Log ID"
    , pp."Order ID"
    , sev.SMRTDTA_ELEM_VALUE  "Procedure Evaluation"
    , sed.ELEMENT_ID
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#11149'
),
...
(SELECT
    ...
    , sed.ELEMENT_ID
    , CASE
        WHEN sev.SMRTDTA_ELEM_VALUE = '0' THEN 'No'
        WHEN sev.SMRTDTA_ELEM_VALUE = '1' THEN 'Yes'
    END AS "Heart Rate Change"
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#13003'
),
...
FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
WHERE sed.ELEMENT_ID = 'EPIC#13005'
),
...
FROM
    (SELECT
        pp."Log ID"
        , pp."Order ID"
        , sev.LINE "Complication"
        , sev.SMRTDTA_ELEM_VALUE "Complication Value"
        , sed.ELEMENT_ID
    FROM PAT_POP pp
    INNER JOIN SMRTDTA_ELEM_DATA sed ON pp."Order ID" = sed.RECORD_ID_NUMERIC
    INNER JOIN SMRTDTA_ELEM_VALUE sev ON sed.HLV_ID = sev.HLV_ID
    WHERE sed.ELEMENT_ID = 'EPIC#13008'
    )
