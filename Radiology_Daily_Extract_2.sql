...
, --002 ADDED
 Out_Room  as        
(SELECT orl.LOG_ID
, convert(varchar,olct.TRACKING_TIME_IN,101)  +' '+  convert(varchar(5),olct.TRACKING_TIME_IN,108)         [Out Room]
, olct.line
FROM  or_log orl
join OR_LOG_CASE_TIMES olct on olct.LOG_ID = orl.LOG_ID 
left outer join ZC_OR_PAT_EVENTS zcpe on zcpe.TRACKING_EVENT_C = olct.TRACKING_EVENT_C 
...
SELECT
	...
	isnull(vis.ACCESSION_NUM,'')																									        [ACCESSION]
	...
	,  [ORD_SCHED_NOTES] =  isnull(STUFF((
         SELECT (', '+ ord_sched_notes.order_sched_notes  )       --002 ADDED - to see if this corrected error in SSIS pkg
		 --((char(10))+ ord_sched_notes.order_sched_notes  )      --002 REMOVED - getting a column length exceeded msg in SSIS pkg
             FROM ord_sched_notes 
			  WHERE ord_sched_notes.order_id = vis.ORDER_ID                     
		  FOR XML PATH('')
         ), 1, 1, ''),'')                                                                                                                                                   --001
    , isnull(in_room.[In Room],'')                                     [Time In]                                                                                                       --002                                                  
...
FROM (SELECT oraud.ORDER_PROC_ID
	...
	WHERE  oraud.AUDIT_TM between @start and @end
...
JOIN CLARITY_LOC loc on vis.PERFORMING_LOC_ID = loc.LOC_ID  and loc.SERV_AREA_ID = @sa 
