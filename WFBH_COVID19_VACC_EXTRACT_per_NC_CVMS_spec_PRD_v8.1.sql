...
----------  legit patients received doses prior to 3/9/2021
-------------------------------------------------------------------- 
                    SELECT  
                                TRUNC( EPIC_UTIL.EFN_DIN( '25-nov-2020' ))  AS START_DATE   
                              , TRUNC( EPIC_UTIL.EFN_DIN( '25-nov-2020' ))  AS REPORT_BEGIN_DATE                      
...
--                              , TRUNC( EPIC_UTIL.EFN_DIN( '09-MAR-2021' ))  AS REPORT_BEGIN_DATE                                  
                              , TRUNC( EPIC_UTIL.EFN_DIN( 'T' ))            AS END_DATE 
                               , TRUNC( EPIC_UTIL.EFN_DIN( '21-JAN-2022' ))  AS   DOSE_LQL_ID_CHANGE_DATE  
                     FROM   DUAL   
                     ) 
...
--uncommented on 6/30/2021
----------comment out aae 5/11/2021
            , CASE
                WHEN  LOC2.LOC_id IN (  
...
----------comment out aae 5/11/2021
            , CASE 
                WHEN ENC_DEP.rpt_grp_three  IS NOT NULL   THEN    ENC_DEP.rpt_grp_three  ----added aae05202020                                       
...
--
    ...
    ,  A.MEDICATION_ID
     ,RX_NDC.NDC_CODE scanned
     , case 
            when A.NDC_CODE  like '59267-1000%'    then '59267-1000-02'
            when A.NDC_CODE  like '59267-1000-03'  then '59267-1000-03'         
            when A.NDC_CODE  like '80777-273-99'   then '80777-0273-99'
            when A.NDC_CODE  like '80777-0273-99'  then '80777-0273-99'
 
            
            when A.NDC_CODE  like '80777-273-10'   then '80777-0273-99'----- 
            when A.NDC_CODE  like '80777-0273-10'  then '80777-0273-99'-----
            
            when A.NDC_CODE  like '80777-273-15'   then '80777-0273-99'-----
            when A.NDC_CODE  like '80777-0273-15'  then '80777-0273-99'-----
             
            when A.NDC_CODE  like '80777-0273-98' then '80777-0273-98'-----
            when A.NDC_CODE  like '80777-273-98'  then '80777-0273-98'-----
             
            when A.NDC_CODE  like '59676-580%'   then '59676-0580-15'
            when A.NDC_CODE  like '59676-0580%'  then '59676-0580-15'
            
            when A.NDC_CODE  like '59267-1055%'   then '59267-1055-04' 
            when A.NDC_CODE  like '59267-01055%'  then '59267-1055-04' 
            
            when A.NDC_CODE  like '59267-1025%'   then '59267-1025-04' 
            when A.NDC_CODE  like '59267-01025%'  then '59267-1025-04' 
...
---- added below 20211006ae   
---updated by  aae10-29-2021 for IMM PEDS PFIZER DOSE 
--            ,   ( SELECT  DISTINCT 
--            
--                            case
--                                when CL_QANSWER_QA.QUEST_ID  in (  '21061003'    ---	MR IMM CURRENT DOSE NUMBER
--                                                                  ,'2106100301'  ---	MR IMM CURRENT DOSE NUMBER
--                                                                  ,'2106100304'  ---  IMM PEDS PFIZER DOSE
--                                                                  ) and  CL_QANSWER_QA.quest_answer > 2 then '3'
--                                                                  
--                                when CL_QANSWER_QA.QUEST_ID  in (  '21061003'    ---	MR IMM CURRENT DOSE NUMBER
--                                                                  ,'2106100301'  ---	MR IMM CURRENT DOSE NUMBER
--                                                                  ,'2106100304'  ---  IMM PEDS PFIZER DOSE 
--                                                                  ) and  CL_QANSWER_QA.quest_answer < 3 then  CL_QANSWER_QA.quest_answer                                     
--                           end   quest_answer                           
--                          
--                  FROM  CL_QANSWER_QA 
--                 WHERE  1=1
--                   AND  CL_QANSWER_QA.QUEST_ID   in (  '21061003' ,'2106100301'  ---	MR IMM CURRENT DOSE NUMBER	---- Which dose in the series is being administered
--                                                      , '2106100304'             ---- LQL	 for WH IMM PEDS PFIZER DOSE  ----  added by  aae10-29-2021
--                                                      )
--                   AND  CL_QANSWER_QA.ANSWER_ID = immune.IMM_ANSWER_ID
--                                 )                                            CURRENT_DOSE_Number   ---- Which dose in the series is being administered                                   
...
--                                     DESC )    dose_RANK  --- ORDER OF DOSE  GRABING THE FIRST ONE 
            ...
            , qa_dose.QUEST_ID   
            , CASE                  
                  WHEN  QA_DOSE.QUEST_ID  =  '21061003'    ---	MR IMM CURRENT DOSE NUMBER
                        THEN  QA_DOSE.QUEST_ANSWER
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100301'  ---	MR IMM CURRENT DOSE NUMBER
                         AND QA_DOSE.QUEST_ANSWER < 4   THEN  QA_DOSE.QUEST_ANSWER
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100301'  ---	MR IMM CURRENT DOSE NUMBER
                         AND QA_DOSE.QUEST_ANSWER = 4   THEN  '3'   
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100301'  ---	MR IMM CURRENT DOSE NUMBER
                         AND QA_DOSE.QUEST_ANSWER > 4   THEN  QA_DOSE.QUEST_ANSWER 
 
----------------------------------------------------------------------------
--begin changes 2022-01-10 for lql dose question
-----CHANGE IN DOSE LQL                   
----------------------------------------------------------------------------
--                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302'   ---  WH IMM BOOSTER DOSE 
--                        THEN  QA_DOSE.QUEST_ANSWER
----------------------------------------------------------------------------
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date < ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                        THEN  QA_DOSE.QUEST_ANSWER
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 1  THEN '2'
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 2  THEN '3'                    
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 3  THEN '4'        
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 4  THEN '5'  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 5  THEN '6'   
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100302' and immune_date >= ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS )  ---  WH IMM BOOSTER DOSE 
                     AND    QA_DOSE.QUEST_ANSWER = 6  THEN '7'   
----------------------------------------------------------------------------                     
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100303'   ---  J and J
                        THEN  QA_DOSE.QUEST_ANSWER 
                  
                  WHEN  QA_DOSE.QUEST_ID  =  '2106100304'   ---  IMM PEDS PFIZER DOSE  
                        THEN  QA_DOSE.QUEST_ANSWER 
                        
                  ELSE  QA_DOSE.QUEST_ANSWER                                  
               END                 CURRENT_DOSE_Number 
               
               
----  added IMMUNE.IMMUNE_DATE desc  statement below 20211025ae  
----In the case of the errors - the Dose_Rank gets the wrong index/sort value because of the way the users entered in the data.          
            , ROW_NUMBER() 
               OVER ( PARTITION BY  IMMUNE.PAT_ID  
                          ORDER BY  IMMUNE.PAT_ID , IMMUNE.IMMUNE_DATE asc, IMMUNE.IMMUNE_ID 
                                     ASC   )   dose_num   --- USE TO FIND MOST RECENT I.E. MAX NUMBER OF DOSE          
            , ROW_NUMBER() 
               OVER ( PARTITION BY  IMMUNE.PAT_ID  
                          ORDER BY  IMMUNE.PAT_ID , IMMUNE.IMMUNE_DATE desc , IMMUNE.IMMUNE_ID 
                                     DESC )    dose_RANK  --- ORDER OF DOSE  GRABING THE FIRST ONE 
----  added IMMUNE.IMMUNE_DATE desc  statement above  20211025ae ---------------------------------------
            ...
            , initcap( CASE WHEN ( UPPER( CLARITY_POS.CITY) ='WINSTON SALEM'  AND zc_cty.NAME IS NULL ) THEN 'Forsyth'
             ...
             ,IMMUNE.entry_user_id    
             ,immune.immnztn_dose_amount
             ,immune.mfg_c
    ...
    FROM IMMUNE    
          ...
          LEFT JOIN clarity_lot               on    immune.lot                   =    clarity_lot.lot_num 
                                              ...
                                              and  RX_NDC.NDC_ID   = clarity_lot.NDC_NUM_ID
          LEFT JOIN CLARITY_NDC_CODES  A      ON RX_NDC.NDC_CODE = A.NDC_CODE  
     	...
     	left join  ( 
                     SELECT DISTINCT   
                      ...
                      FROM (      
                            select  distinct
                                        snp.PAT_ID
                                      ,snp.UTIL_SNAP_PROV_ID
                                      , snp.pat_enc_csn_id
--                                      , snp.contact_date
--                                      , ROW_NUMBER() 
--                                         OVER ( PARTITION BY    snp.PAT_ID
--                                                    ORDER BY    snp.pAT_ID , snp.contact_date  DESC  )    SER_rank      ----MOST RECENT APPT THAT MEETS CRITERIA                       
                               from   pat_enc    pe 
                                         join  APPT_UTIL_SNAPSHOT snp    on    pe.PAT_ENC_CSN_ID     = snp.pat_enc_csn_id  
                              where 1=1
                                AND  pE.CONTACT_DATE  BETWEEN   ( SELECT START_DATE FROM MYPARAMS )    and    ( SELECT END_DATE FROM MYPARAMS )
                                and  pe.appt_prc_id                 IN ('586', '587', '588')  
                                and  snp.UTIL_SNAP_CHANGE  = '0'
                                and  snp.UTIL_SNAP_PROV_ID = '8274'  ----UNITED HEALTH CENTERS @ PETERS CREEK PARKWAY
                             )  
                    WHERE 1=1
--                    WHERE  SER_RANK =1    --- REMOVE THIS LINE - BECUASE YOU WANT TO IDENTIFY ALL CSNS RELATED TO THE SCHEDULED 8274 PROVIDER 
												 )  tmpser    on   IMMUNE.IMM_CSN                =    tmpser.PAT_ENC_CSN_ID    
 
                          
      LEFT JOIN CL_QANSWER_QA    qa_dose    ON  immune.IMM_ANSWER_ID     =  qa_dose.ANSWER_ID 
                                           and   qa_dose.QUEST_ID  in (    '21061003'    ---	MR IMM CURRENT DOSE NUMBER
                                                                          ,'2106100301'  ---	MR IMM CURRENT DOSE NUMBER
                                                                          ,'2106100302'  ---- WH IMM BOOSTER DOSE 
                                                                          ,'2106100303'   ---  J and J
                                                                          ,'2106100304'  ---  IMM PEDS PFIZER DOSE  ----  added by  aae10-29-2021
                                                                              )  
...
, BASE_POPx AS (
                SELECT DISTINCT
                        ...
                        ,	IMMUNE_DATE
                        ,	VAX_EVENT_ID
 --                       ,	CURRENT_DOSE_NUMBER              ----ADDED   20210811
 
 ------------------------------------------------------
 ------ADDED   2022-01-10
  ------------------------------------------------------
--                       , CASE
--                            WHEN   QUEST_ID  =  '2106100302'    ---  WH IMM BOOSTER DOSE
--                            THEN DOSE_NUM
--                         ELSE TO_NUMBER(CURRENT_DOSE_NUMBER)
--                         END 	CURRENT_DOSE_NUMBER
                       , CASE
                            WHEN   QUEST_ID  =  '2106100302'  and  	IMMUNE_DATE  <  ( SELECT DOSE_LQL_ID_CHANGE_DATE    FROM MYPARAMS ) ---  WH IMM BOOSTER DOSE
                            THEN DOSE_NUM
                         ELSE TO_NUMBER(CURRENT_DOSE_NUMBER)
                         END 	CURRENT_DOSE_NUMBER
 ------------------------------------------------------                         
                        ...
                        ,	VAX_ADMIN_SITE
                        ,	VAX_ROUTE
                        ,	VAX_ADMIN_PROVIDER_NAME
                        ,	VIS_PUBLICATION_DATE
                        ,	VIS_DATE_GIVEN_TO_RECIPIENT
                        ,	ADVERSEREACTIONCONSENT
                        ,	VAX_REACTION
                        ,	VAX_REACTION_DESC
                        ,	EXCLUDE_SCH_TMP_PROV_ID
                        ,	TEMPLATE_PROV_ID
                        ,	TEMPLATE_PROV_NAME
                        ,	TEMPLATE_PROV_TYPE
                        ,	VAX_EVENT_LAST_MODIFIED
                        ,	PRODUCT_NAME
                        ,	EPIC_DEPARTMENT
                        ,	VISIT_TYPE
                        ,	PRC_ID
             ...
             , mfg_c
                  FROM IMM_DAT 
                 WHERE 1=1
                   AND IMM_DAT.entry_user_id <> 'EDIPSVACIN'     ---  AAE2021-08-19:  USED TO IDENTIFY INTERFACED POPULATION 
--                   AND ( DOSE_RANK = 1                        ---- LOOKUP THE MOST RECENT VAX TO FIND DOSE NUMBER
--                         or  VISIT_TYPE LIKE '%2ND%'         ----REMOVED  20211006
--                         OR  ( DOSE_RANK = 1  AND  upper(imm_dat.PRODUCT_NAME)   like upper( 'JANSSEN%COV%'  )  )
--                         )  
...
, BASE_POP AS ( 
                SELECT DISTINCT
                        ...
                        ,	VAX_EVENT_ID
                        ,	CURRENT_DOSE_NUMBER              ----ADDED   20210811
                        , case when BASE_POPx.CURRENT_DOSE_NUMBER  is not null then  to_char(BASE_POPx.CURRENT_DOSE_NUMBER)
                               else  to_char(BASE_POPx.DOSE_NUM  )
                          end                               as    DOSE_NUM    ------ UPDATED DOSE_num
                        ,	DOSE_RANK
                        ,	LOT_NUMBER
                        ,	VAX_EXPIRATION_DATE
                        ,	VAX_ADMIN_SITE
...
--aae10/28/2021   uncomment code below per Moses -- We are in a situation where there are 2 or more NDC�s associated with the vaccinations so calculating the NDC is no longer feasible                            
                         , BASE_POP.ndc  
--aae10/28/2021   comment out    below:                      
--                            , case 
...
----    CASE STATEMENT PERFORMED BY ORDER OF PRECEDENCE
-------------------------------------------------------------------- 
                            ,   CASE 
                                    WHEN  ( BASE_POP.CURRENT_DOSE_NUMBER is not null  or BASE_POP.CURRENT_DOSE_NUMBER > 0 )
                                                                                    THEN   'Dose ' || trim(BASE_POP.CURRENT_DOSE_NUMBER) || ' Administered' 
                                    ...
                                    WHEN (
 ---aae10-29-21 updating logic because we have booster shots now
                                           ---BASE_POP.dose_num  > 1    OR  
                                           BASE_POP.VISIT_TYPE LIKE '%2ND DOSE%' )  THEN   'Dose 2 Administered'

 ---aae10-29-21 addedline below  for when no LQL question provided                                         
                                    WHEN  ( BASE_POP.CURRENT_DOSE_NUMBER is null  and  BASE_POP.dose_num  > 0 )
                                                                                    THEN   'Dose ' || trim(BASE_POP.dose_num ) || ' Administered'                                            
 ---aae10-29-21   end of change
...
---RE PURPOSE OF FIELD BELOW...  PER CVMS SPEC NC COVID-19 Vaccine Reporting File Spec v8_20211015.XLSX   
...
--                               END                             Recipient_type  
------------------------------------repurposed to denote whether it is a FULL or HALF dose of the vaccine
                        ,     CASE
                                    WHEN ( ---logic for Moderna half dose booster
                                               BASE_POP.mfg_c = '43'
                                           and BASE_POP.immnztn_dose_amount = '0.25'
                                           )   THEN 'HALF'
                                    
                                    else  'FULL'
                              END    Quantity_Administered
...
------------------AAE20210811      CHANGE DOSE LOGIC TO LOOK AT SURVEYED CURRENT DOSE FIRST 
                       ,base_pop.dose_num   patient_doses  
--                      , FAILED_iMM_CSN_CHECK
                       ,BASE_POP.IMMUNE_DATE  
--                       ,BASE_POP.Lookup_admin_name
--                       ,BASE_POP.Lookup_vtrcks_prov_pin
                    FROM  BASE_POP      
...
----     and  Quantity_Administered ='HALF'
----      and  patient_doses > 2

-- 	and    epic_department ='C2311 01 URGENT CARE [1009301001]'
--    and trunc( IMMUNE_DATE) ='29-DEC-21'

--     and  (    visit_type like upper('COVID%Vaccine%')
--           or  visit_type like upper('%2nd%dose%')
--            ) 
      ORDER BY     recip_id , IMMUNE_DATE
      
      
-- 2/25/2022 resubmits - VAX_EVENT_ID
--  	26845150
--		26815343
--		26833220