...
, IMM_DAT AS (    
  SELECT    distinct 
            ...
            , ''                                                                org_id          ---- parent vfc --1/26/2021   from spec per CVMS  
            ,  ENC_DEP.specialty_dep_c                                          admin_type      ---- this will be vfc for admin  location  
            , CASE
                WHEN  LOC2.LOC_id IN (  
                                         '100000'--	  PARENT WAKE FOREST
                                        ,'100001'--		PARENT LEXINGTON
                                        ,'100002'--		PARENT DAVIE
                                        ,'100004'--		PARENT CORNERSTONE
                                       )  THEN 'Wake Forest Baptist Medical Center'  --SUPPLY CHAIN / inventory location 
                WHEN  LOC2.LOC_id IN (  '100005'--		PARENT WILKES 
                                       )  THEN 'Wilkes Medical Center'               --SUPPLY CHAIN / inventory location  
                WHEN  LOC2.LOC_id IN (  '100009'--		PARENT HIGH POINT 
                                       )  THEN 'High Point Medical Center'            --SUPPLY CHAIN / inventory location                                          
                ELSE                           'Wake Forest Baptist Medical Center'   --SUPPLY CHAIN / inventory location                        
              END                                                               admin_name    --- parent vfc --1/26/2021   from spec per CVMS 
            , ''                                                                admin_id      --- vfc for admin  location  '40166742'  --1/26/2021 remove from spec per CVMS 
            , CASE
                WHEN  LOC2.LOC_id IN (  
                                         '100000'--	  PARENT WAKE FOREST
                                        ,'100001'--		PARENT LEXINGTON
                                        ,'100002'--		PARENT DAVIE
                                        ,'100004'--		PARENT CORNERSTONE
                                       )  THEN '34C001'               --SUPPLY CHAIN / inventory location 
                WHEN  LOC2.LOC_id IN (  '100005'--		PARENT WILKES 
                                       )  THEN '97C001'               --SUPPLY CHAIN / inventory location  
                WHEN  LOC2.LOC_id IN (  '100009'--		PARENT HIGH POINT 
                                       )  THEN '34C006'               --SUPPLY CHAIN / inventory location                                          
                ELSE                           '34C001'               --SUPPLY CHAIN / inventory location                        
              END                                                               vtrcks_prov_pin     --- vfc for admin  location  '40166742'  --1/26/2021 remove from spec per CVMS 
            , NDC_CODE                                                          ndc  
------------------------------------------------------------------------------------
--  PER MOSES - BE PREPARED TO USE   immunization_time   vs IMMUNE_DATE 
...
--            , CLARITY_SER.prov_name                                             vax_admin_provider_name       --1/26/2021 USE EMPLOYEE instead of SER for nurses
            , E.EMP_NAME                                                        vax_admin_provider_name   
--            , ''                                                              Provider_License_Number        --1/26/2021 remove from spec per CVMS
...
---- USE COALESCE STATEMENT FOR IMM AND ORD TO HELP IDENTIFY LOCATION TO USE FOR VAX 
          ...
          LEFT JOIN (
                        select  distinct
                         ...
                         where  1=1 ------------USER_ID=upper('aechard')-----'PKBROWN'  --KNIGHT-BROWN, PHYLLIS D.   ---PKBROWN            
                  ...
                  SELECT * 
                    FROM (
                          SELECT  DISTINCT
                                  ...
                                  , CASE 
                                         WHEN ZC_ETHNIC_GROUP.NAME IS NULL                      THEN   'Other'
                                         WHEN ZC_ETHNIC_GROUP.NAME = 'Patient Refused'          THEN   'Other'
                           ...
                           SELECT * 
                             FROM (
                                  select DISTINCT
                                        ...
                                        , ROW_NUMBER() 
                                           OVER ( PARTITION BY  ord.pat_id
                                                      ORDER BY  ord.pat_id ,  RES_DB.RES_INST_VALIDTD_TM  DESC )   rank   --- USE TO FIND MOST RECENT  
                                ...
                                SELECT * 
                                  FROM (
                                      SELECT  distinct
                                            ...
                                            , ROW_NUMBER() 
                                               OVER ( PARTITION BY  ord.pat_id
                                                          ORDER BY  ord.pat_id , ord_r.RESULT_DATE  DESC )   rank   --- USE TO FIND MOST RECENT  
                                  ...
                                  WHERE  RANK =1
                                  )      LAB   ON   LAB.pat_id   =    BASE_POP.PAT_ID
)
        ...
        SELECT * 
         ...
         WHERE  1=1 
                 AND  (   MEM_EFF_TO_DATE IS NULL
                       OR TRUNC(cml.MEM_EFF_TO_DATE) >= TRUNC ( SYSDATE )
                       )
               )
       WHERE RANK =1
      )
...
--                            , BASE_POP.admin_id   --- vfc for admin  location  '40166742'
                            ...
                            ,   CASE 
                                  ...
                                  WHEN BASE_POP.dose_num  = 2     THEN   'Dose 2 Administered'           
                                END                    dose_num --- Comorbidity status (Y/N)  
                                 
                            , BASE_POP.lot_number  
                            , TO_CHAR(  BASE_POP.vax_expiration , 'YYYY-MM-DD')  vax_expiration
                            , BASE_POP.vax_admin_site
                            , BASE_POP.vax_route 
                            , BASE_POP.vax_admin_provider_name
--                            , BASE_POP.Provider_License_Number     -------------------------------- dropped per updated spec 1/25/2021
                            , TO_CHAR( BASE_POP.vis_publication_date , 'YYYY-MM-DD') vis_publication_date  --- VIS EUA DATE        
...
--COVID-19 Vaccination Program in Five Phases 
                           ,CASE
                               WHEN (    GRP.OCCUPATION = 'HEALTHCARE'
                                     OR  GRP.INDUSTRY   = 'HEALTHCARE'
                                     )
                                    AND GRP.EMPLOYMENT_STATUS IN ('Full Time','Part Time','Self Employed')                      THEN 'Group 1'---'Phase 1a' 
                                WHEN (
                                      (     GRP.EMPLOYMENT_STATUS IN ('Retired','Full Time','Part Time','Self Employed')    
                                       and  trunc( months_between( BASE_POP.IMMUNE_DATE,  PAT_DEMO.birth_date )/ 12 ) >=65 
                                       )
                                      OR  COMORBIDITY.COMORBIDITIES > 1
                                      )                                                                                         THEN 'Group 2'---'Phase 1b' 
                                WHEN (
                                        trunc( months_between( BASE_POP.IMMUNE_DATE,  PAT_DEMO.birth_date )/ 12 ) >=65
                                       OR ( trunc( months_between( BASE_POP.IMMUNE_DATE,  PAT_DEMO.birth_date )/ 12 )  between 16 and 64  
...
--                                   ELSE   ''
--                              END                                   SEROLOGY 
                            , ' '                                    SEROLOGY       --Required, if known for this recipient.   If Unknown, please leave blank. 
...
--                            , serology.RESULT_DATE                  Date_of_Disease              --- CHECK MEDICAL HX FOR COVID
                            ...
                            , 'None of the above'                    Recipient_type 
                            , 'None of the above'                    INDUSTRY 
